...
class Electrum
{
    ...
    {
        $host = config('app.electrum.host', '127.0.0.1');
        $port = config('app.electrum.port', 7777);
        $username = config('app.electrum.user');
        $password = config('app.electrum.password');
        $electrumURI = "http://" . $username . ':' . $password . "@" . $host . ':' . $port;
        $this->client = new Client([
            'base_uri' => $electrumURI,
        ]);
    }
    ...
    {
        return $this->sendRequest('listaddresses', $data);
    }
